...
//relates to reading from google sheets
...
app.get('/login', function(request, response){
  ...
  if(user_data["name"]==""||user_data["password"]==""){//does not allow either empty username or empty password
    console.log("Incomplete login, send back to index");
    response.status(200);
    response.setHeader('Content-Type', 'text/html')
    ...
    models_user.getAllUsers(function(user_info){
      ...
      for(i=0;i<user_info.length;i++){
        ...
        if(user_info[i]["name"]==user_data["name"]){
          console.log("Username match found for login");
          if(user_info[i]["password"]==user_data["password"]){
            ...
            models_villain.getAllVillains(function(villain_info){
              ...
              console.log("Successful login, going to game page");
              response.status(200);
              response.setHeader('Content-Type', 'text/html');
            ...
            console.log("Password does not match database, sending back to index page");
            response.status(200);
            response.setHeader('Content-Type', 'text/html');
...
app.get('/:user/results', function(request, response){
  ...
  console.log("Beginning of game results request");
  console.log("Weapon choice: "+user_data.weapon);
  console.log("Villain choice: "+user_data.villain_choice);
  ...
  models_user.getAllUsers(function(user_info){
    ...
    models_villain.getAllVillains(function(villain_data){
      ...
      models_user.writeUsersToSheet(user_info);//updates user information
      ...
      models_villain.updateVillain(villain_data);//updates villain information
      //rewrites new villain information to csv file
      console.log("End of game results request");
      response.status(200);
      response.setHeader('Content-Type', 'text/html')
...
app.get('/rules', function(request, response){
  console.log("GET request for rules page");
  response.status(200);
  response.setHeader('Content-Type', 'text/html')
...
app.get('/stats', function(request, response){
  ...
  models_user.getAllUsers(function(user_info){
    //console.log("user_info:"+JSON.stringify(user_info));
    var user_data = user_info;//redundant?
    models_villain.getAllVillains(function(villain_info){
      //console.log("villain_info:"+JSON.stringify(villain_info));
